...
public class RickAndMortyClient {
  ...
  public static EpisodeDetails getEpisodeDetailsWithStatusOkAndReturnEpisodeDetails(
      final RequestSpecBuilder requestSpecBuilder, final GraphQLQuery graphQLQuery, final int httpStatus
  ) {
    return given()
        .spec(requestSpecBuilder.build()).body(graphQLQuery).post("/")
        .then().statusCode(httpStatus)
        .extract()
        .response().as(EpisodeDetails.class);
  }
}
