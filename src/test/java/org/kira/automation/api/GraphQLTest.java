...
public class GraphQLTest extends TestSuiteRunner {
  ...
  public void testGraphQlQuery() {
    ...
    getGraphQlQueryInstance()
      .setQuery(FileUtils.readFileAsString(GRAPHQL_QUERY_PATH + "episode.gql"));
    getExtentTest().log(INFO, "Creating the query variables");
    getGraphQlQueryVariablesObject().put("episodeId", "3");
    getGraphQlQueryInstance().setVariables(EpisodeId.create().episodeId("3").build());
    getExtentTest().log(INFO, "Sending the request for getting episode detail");
    EpisodeDetails episodeDetails =
      RickAndMortyService.getEpisodeDetailsWithStatusOkAndReturnEpisodeDetails(
    ...
    SoftAssertions.assertSoftly(softly -> {
      ...
      softly
        .assertThat(episode.getCreated())
        .as("Episode Created Date")
