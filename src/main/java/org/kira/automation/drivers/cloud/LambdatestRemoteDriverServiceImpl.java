...
public class LambdatestRemoteDriverServiceImpl implements CloudRemoteDriverService {
  ...
  private static final Map<String, LambdatestBrowserConfigRetriever> BROWSER_CONFIG_RETRIEVERS =
    new HashMap<>();
  ...
  public WebDriver getWebDriver(
    ...
    Map<String, String> capabilityMapOptional
  ) {
    LambdatestConfiguration lambdatestConfiguration = configuration
      .getWeb()
      .getCloud()
      .getProvider()
      .getLambdatestConfiguration();

    String username = Optional.ofNullable(lambdatestConfiguration.getUserName()).orElse(
      System.getProperty(CLOUD_USERNAME)
    );
    String password = Optional.ofNullable(lambdatestConfiguration.getAccessKey()).orElse(
    ...
    try {
      return new RemoteWebDriver(
        new URI(url).toURL(),
        this.getPlatformSpecificCapabilities(
            configuration.getWeb().getCloud(),
  ...
  public MutableCapabilities getPlatformSpecificCapabilities(
    ...
    LambdatestConfiguration lambdatestConfiguration = cloudConfiguration
      .getProvider()
      .getLambdatestConfiguration();
    ...
    CloudBrowserConfiguration browserSpecificCapabilities =
      this.getBrowserSpecificCapabilities(lambdatestConfiguration, browserName);
      ...
      capabilityOptionalMap.ifPresentOrElse(
        ...
        () -> {
          browserOptions.setPlatformName(
            String.format(
              "%s %s",
              browserSpecificCapabilities.getOs(),
  ...
  private CloudBrowserConfiguration getBrowserSpecificCapabilities(
    LambdatestConfiguration lambdatestConfiguration,
    String browserName
  ) {
